# 生成C-arm风格的X光图像
 ...
 - 准备几张（`~3-5`）图片，放在`carm_vertebrae`文件夹下，需要宽高大小一致
 - 直接运行`bash train_dreambooth_carm.sh`进行训练，参数`instance_data_dir`即为训练数据的目录，`instance_prompt`是提示信息，用于进行条件生成，需要一个罕见词，用于提示数据集的信息，如使用`carm_vertebrae`或者`[V]`。
 - 训练完成后，执行`python inference_dreambooth.py`进行图像采样：训练完成后，会在outouts目录下保存一些模型文件，通过`prompt`控制生成条件。

由于需要在hugging face上下载模型文件，可以通过下面镜像进行连接：
~~~bash
vim ~/.bashrc
......
# 添加下面一行
...
以下是原始的readme信息，可参考进行安装和或其他帮助。代码在`diffusers==0.30.0.dev0`版本上进行试验通过。
